...
"""Default configuration parameters (overridable in configs/*.yaml)
"""
...
cfg.checkpoint.save_task_end = False    # To only save checkpoints at task end
########################################################################################################################
### SAVE
########################################################################################################################
cfg.save = CN()
...
cfg.model.pretrained_model_dir = ''         # Directory where pretrained models will be downloaded from torch hub
########################################################################################################################
### MODEL.OPTIMIZER
########################################################################################################################
...
cfg.model.optimizer.depth.name = cfg.model.optimizer.name
...
cfg.model.optimizer.pose.name = cfg.model.optimizer.name
########################################################################################################################
### MODEL.SCHEDULER
########################################################################################################################
cfg.model.scheduler = CN()
cfg.model.scheduler.name = 'SequentialLR'                               # Scheduler name
...
cfg.model.scheduler.milestones = [cfg.model.scheduler.step_size]        # The milestone epochs where the lr is multiplied by gamma (MultiStepLR)
cfg.model.scheduler.sequential = CN()
cfg.model.scheduler.sequential.names = ['StepLR']                  # To be used with SequentialLR
cfg.model.scheduler.sequential.stepsizes = [cfg.model.scheduler.step_size] * len(cfg.model.scheduler.sequential.names)
cfg.model.scheduler.sequential.gammas = [cfg.model.scheduler.gamma] * len(cfg.model.scheduler.sequential.names)
########################################################################################################################
### MODEL.PARAMS
########################################################################################################################
cfg.model.params = CN()
...
cfg.model.loss.multimem_image_consistency_crop = False          # Apply image consistency loss on the random crop of image
cfg.model.loss.multimem_image_consistency_loss_weight = 1.0     # Weight of image consistency loss
########################################################################################################################
### MODEL.DEPTH_NET
########################################################################################################################
...
cfg.model.depth_net.extra_config = ''           # Additional config file (eg. used for Transformers)
cfg.model.depth_net.transformer_model = ''      # Model used when depth net is transformer
cfg.model.depth_net.transformer_features = 96   # Number of features when depth net is a transformer
########################################################################################################################
### MODEL.POSE_NET
########################################################################################################################
...
cfg.model.pose_net.learn_intrinsics = False     # Predict camera intrinsics
cfg.model.pose_net.extra_config = ''            # Additional config file (eg. used for Transformers)
cfg.model.pose_net.transformer_model = ''       # Model used when pose net is transformer
########################################################################################################################
### DATASETS
########################################################################################################################
cfg.datasets = CN()
...
cfg.datasets.augmentation.kitti_crop = False                # Crop data like KITTI
...
########################################################################################################################
### DATASETS.TRAIN.CONTINUAL
########################################################################################################################
cfg.datasets.train.continual = CN()
cfg.datasets.train.continual.enabled = False                                        # If continual learning then the datasets sequentially trained
cfg.datasets.train.continual.strategy = ''                                          # If using a special CL strategy mention here else leave empty
cfg.datasets.train.continual.rehearsal_batch_size = cfg.datasets.train.batch_size   # Batch size of data from buffer in rehearsal CL
cfg.datasets.train.continual.buffer_size = 0                                        # Buffer size for CL
cfg.datasets.train.continual.begin_reg_epoch = 0                                          # The epoch from which the regularization should be applied
cfg.datasets.train.continual.reg_weight = 0.1                                       # Weight on consistency loss between plastic and stable model
cfg.datasets.train.continual.stable_update_freq = 0.07                              # Update frequency of stable model.
cfg.datasets.train.continual.stable_alpha = 0.999                                   # Alpha used in mean teacher update equation for stable model
########################################################################################################################
### DATASETS.VALIDATION
########################################################################################################################
