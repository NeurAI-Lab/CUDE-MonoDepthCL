...
class ModelCheckpoint:
    def __init__(self, filepath=None, monitor='val_loss',
                 ...
                 s3_path='', s3_frequency=5, save_task_end=False):
        super().__init__()
        ...
        self.kth_best_model = ''
        self.best = 0
        self.save_task_end = save_task_end
        # Monitoring modes
    ...
    def _save_model(self, filepath, model):
        ...
        torch.save({
            ...
            'depth_optimizer': model.depth_optimizer.state_dict(),
            'pose_optimizer': model.pose_optimizer.state_dict(),
            'depth_scheduler': model.depth_scheduler.state_dict(),
            'pose_scheduler': model.pose_scheduler.state_dict(),
        }, filepath)
    ...
    def format_checkpoint_name(self, task, tepoch, epoch, metrics):
        metrics['task'] = task
        metrics['tepoch'] = tepoch # task epoch
        metrics['epoch'] = epoch
    ...
    def get_task_epoch(self, current_epoch, max_epochs):
        cumulative_array = np.append([0], np.cumsum(max_epochs))
        # Index of first cumulative max_epochs (incl. 0) less than current epoch
        i = np.searchsorted(cumulative_array - 1, current_epoch)
        current_epoch_task = current_epoch - cumulative_array[i - 1]
        max_epochs_task = max_epochs[i - 1]
        return current_epoch_task

    def check_and_save(self, model, metrics):
        # Check saving interval
        epoch = model.current_epoch
        task = model.current_task
        max_epochs = model.config.arch.max_epochs
        tepoch = self.get_task_epoch(epoch, max_epochs)

        if self.save_task_end and epoch not in np.cumsum(max_epochs) - 1:
            return
        ...
        # Prepare filepath
        filepath = self.format_checkpoint_name(task, tepoch, epoch, metrics)
