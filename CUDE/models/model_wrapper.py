...
from CUDE.datasets.transforms import get_transforms
from CUDE.utils.depth import inv2depth, post_process_inv_depth, compute_depth_metrics
from CUDE.utils.horovod import print0, world_size, rank, on_rank_0
from CUDE.utils.image import flip_lr
from CUDE.utils.load import load_class, load_class_args_create, \
    load_network, filter_args
from CUDE.utils.logging import pcolor
from CUDE.utils.reduce import all_reduce_metrics, reduce_dict, \
    create_dict, average_loss_and_metrics, compute_cl_metrics, create_cl_dict
from CUDE.utils.save import save_depth
from CUDE.models.model_utils import stack_batch
...
class ModelWrapper(torch.nn.Module):
    ...
    def __init__(self, config, resume=None, logger=None, load_datasets=True):
        ...
        config_filepath = os.path.join(os.path.dirname(config.checkpoint.filepath), 'config.yaml')
        ...
        if self.config.datasets.train.continual.enabled:
            self.all_test_metrics = {}

        # Model, optimizers, schedulers and datasets are None for now
        self.model = self.depth_optimizer = self.pose_optimizer = self.scheduler = None
        ...
        self.results_filepath = os.path.join(os.path.dirname(config.checkpoint.filepath), 'results_{}.yaml')
        self.results_yaml = {'': CN()}
    ...
    def prepare_model(self, resume=None):
        ...
        self.model = setup_model(self.config.model, self.config.prepared,
                                 image_shape=self.config.datasets.augmentation.image_shape,
                                 learn_intrinsics=self.config.model.pose_net.learn_intrinsics)
    ...
    def logs(self):
        ...
        params = OrderedDict()
        for param in self.depth_optimizer.param_groups:
            params['{}_learning_rate'.format(param['name'].lower())] = param['lr']
        for param in self.pose_optimizer.param_groups:
            params['{}_learning_rate'.format(param['name'].lower())] = param['lr']
    ...
    def progress(self):
        ...
        return self.current_epoch / sum(self.config.arch.max_epochs)
    ...
    def configure_optimizers(self):
        ...
        pose_params = []
        # Load optimizer
        depth_optimizer = getattr(torch.optim, self.config.model.optimizer.depth.name)
        pose_optimizer = getattr(torch.optim, self.config.model.optimizer.pose.name)
        # Depth optimizer
        if self.depth_net is not None:
            depth_params.append({
                'name': 'Depth',
                'params': self.depth_net.parameters(),
                **filter_args(depth_optimizer, self.config.model.optimizer.depth)
            })
        # Pose optimizer
        if self.pose_net is not None:
            pose_params.append({
                ...
                **filter_args(pose_optimizer, self.config.model.optimizer.pose)
            })
        # Create optimizer with parameters
        depth_optimizer = depth_optimizer(depth_params)
        pose_optimizer = pose_optimizer(pose_params)

        # Load and initialize scheduler
        scheduler = getattr(torch.optim.lr_scheduler, self.config.model.scheduler.name)
        if self.config.model.scheduler.name == "SequentialLR":
            sequential_params = self.config.model.scheduler.sequential
            depth_schedulers = [getattr(torch.optim.lr_scheduler, s) for s in sequential_params.names]
            pose_schedulers = [getattr(torch.optim.lr_scheduler, s) for s in sequential_params.names]
            depth_schedulers = [
                ds(depth_optimizer, step_size=sequential_params.stepsizes[i], gamma=sequential_params.gammas[i])
                for i, ds in enumerate(depth_schedulers)]
            pose_schedulers = [
                ps(pose_optimizer, step_size=sequential_params.stepsizes[i], gamma=sequential_params.gammas[i])
                for i, ps in enumerate(pose_schedulers)]

            # Sequential Schedulers
            depth_scheduler = scheduler(optimizer=depth_optimizer, schedulers=depth_schedulers,
                                        **filter_args(scheduler, self.config.model.scheduler))
            pose_scheduler = scheduler(optimizer=pose_optimizer, schedulers=pose_schedulers,
                                       **filter_args(scheduler, self.config.model.scheduler))
        else:
            depth_scheduler = scheduler(depth_optimizer, **filter_args(scheduler, self.config.model.scheduler))
            pose_scheduler = scheduler(pose_optimizer, **filter_args(scheduler, self.config.model.scheduler))

        if self.resume:
            if 'depth_optimizer' in self.resume:
                depth_optimizer.load_state_dict(self.resume['depth_optimizer'])
            elif 'optimizer' in self.resume:
                depth_optimizer.load_state_dict(self.resume['optimizer'])
            if 'pose_optimizer' in self.resume:
                pose_optimizer.load_state_dict(self.resume['pose_optimizer'])
            elif 'optimizer' in self.resume:
                pose_optimizer.load_state_dict(self.resume['optimizer'])

            if 'depth_scheduler' in self.resume:
                depth_scheduler.load_state_dict(self.resume['depth_scheduler'])
            elif 'scheduler' in self.resume:
                depth_scheduler.load_state_dict(self.resume['scheduler'])
            if 'pose_scheduler' in self.resume:
                pose_scheduler.load_state_dict(self.resume['pose_scheduler'])
            elif 'scheduler' in self.resume:
                pose_scheduler.load_state_dict(self.resume['scheduler'])
        ...
        return depth_optimizer, pose_optimizer, depth_scheduler, pose_scheduler
    ...
    def training_step(self, batch, *args):
        ...
        output = self.model(batch,
                            ...
                            max_epochs=self.config.arch.max_epochs,
    ...
    def validation_step(self, batch, *args, task_idx=0):
        """Processes a validation batch."""
        output = self.evaluate_depth(batch, task_idx=task_idx)
        if self.
    ...
    def test_step(self, batch, *args, task_idx=0):
    ...
    def validation_epoch_end(self, output_data_batch, name=''):
        """Finishes a validation epoch."""

        # Reduce depth metrics
        metrics_data = all_reduce_metrics(
            output_data_batch, self.validation_dataset, name=name + self.metrics_name)

        # Create depth dictionary
        metrics_dict = create_dict(
            metrics_data, self.metrics_keys, self.metrics_modes,
            self.config.datasets.validation, name=name + self.metrics_name)

        cl_metrics_dict = None
        # if training continually and at end of task
        if self.config.datasets.train.continual.enabled and self.current_epoch in np.cumsum(self.config.arch.max_epochs) - 1:
            print("Computing CL metrics")
            if name not in self.all_test_metrics.keys():
                self.all_test_metrics[name] = {}
            for mode in self.metrics_modes:
                if name + self.metrics_name + mode not in self.all_test_metrics[name].keys():
                    self.all_test_metrics[name][name + self.metrics_name + mode] = []
                test_metrics = torch.stack([metrics_data[i][name + self.metrics_name + mode] for i in range(len(metrics_data))]).numpy()
                self.all_test_metrics[name][name + self.metrics_name + mode].append(test_metrics)
            cl_metrics_data = compute_cl_metrics(self.all_test_metrics[name])
            cl_metrics_dict = create_cl_dict(cl_metrics_data, self.metrics_keys)
            if hasattr(self, 'results_yaml') and self.current_epoch == np.cumsum(self.config.arch.max_epochs)[-1] - 1:
                if name not in self.results_yaml:
                    self.results_yaml[name] = CN()
                for key, value in metrics_dict.items():
                    self.results_yaml[name][key] = value
                for key, value in cl_metrics_dict.items():
                    self.results_yaml[name][key] = value.item()
                import yaml
                with open(self.results_filepath.format(name), 'w') as f:
                    print(f"dumping yaml {name}")
                    yaml.dump(self.results_yaml[name], f, default_flow_style=False)
        ...
        if self.
            ...
            if cl_metrics_dict:
                self.logger.log_metrics({
                    **cl_metrics_dict, 'global_step': self.current_epoch + 1,
                })
        return {
            **metrics_dict
        }
    ...
    def test_epoch_end(self, output_data_batch):
        ...
        # if training continually and at end of task
        if self.config.datasets.train.continual.enabled and self.current_epoch in np.cumsum(self.config.arch.max_epochs) - 1:
            self.all_test_metrics.append(metrics_data)
            cl_metrics_data = compute_cl_metrics(self.all_test_metrics)
    ...
    def evaluate_depth(self, batch, task_idx=0):
    ...
    def print_metrics(self, metrics_data, dataset):
        ...
        if (self.depth_optimizer is not None) or (self.pose_optimizer is not None):
...
def setup_model(config, prepared, image_shape=(192, 640), **kwargs):
    """
    Create a model
    ...
    if config.pretrained_model_dir is not '':
        torch.hub.set_dir(config.pretrained_model_dir)
    model = load_class(config.name, paths=['CUDE.models',])(
        **{**config.loss, **kwargs})
    # Add depth network if required
    if model.network_requirements['depth_net']:
        config.depth_net.image_shape = image_shape
        config.depth_net.num_scales = config.loss.num_scales
    ...
    if model.network_requirements['pose_net']:
        config.pose_net.image_shape = image_shape
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        ...
        if config.dataset[i] == 'KITTI':
            from CUDE.datasets.kitti_dataset import KITTIDataset
        ...
        # CITYSCAPES dataset
            ...
            from CUDE.datasets.cityscapes_dataset import CITYSCAPESDataset
            dataset = CITYSCAPESDataset(config.path[i], path_split,
                                        **dataset_args, **dataset_args_i,
                                        )
        # VKITTI2 dataset
        elif config.dataset[i] == 'VKITTI2':
            from CUDE.datasets.vikitti2_dataset import VKITTI2Dataset
            dataset = VKITTI2Dataset(config.path[i], path_split,
                                     **dataset_args, **dataset_args_i,
                                     )
        # NYUv2 dataset
        elif config.dataset[i] == 'NYUv2':
            from CUDE.datasets.nyuv2_dataset import NYUv2Dataset
            dataset = NYUv2Dataset(config.path[i], path_split,
                                   **dataset_args, **dataset_args_i
                                   )
        # DGP dataset
        elif config.dataset[i] == 'DGP':
            from CUDE.datasets.dgp_dataset import DGPDataset
            dataset = DGPDataset(
        ...
        # Image dataset
            ...
            from CUDE.datasets.image_dataset import ImageDataset
    ...
    if mode == 'train' and not config.continual.enabled:
