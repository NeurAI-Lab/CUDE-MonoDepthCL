...
########################################################################################################################
    ...
    def _get_next_file(idx, file):
        ...
        img_type, file_idx = base.split("_")
        return os.path.join(os.path.dirname(file), f"{img_type}_{str(idx).zfill(len(file_idx))}" + ext)
    ...
    def _get_intrinsics(calib_data, idx):
        ...
        intrinsics = calib_data[(calib_data['frame'] == 0) & (calib_data['cameraID'] == 0)]
        K = np.eye(3)
        K[0, 0] = intrinsics['K[0,0]']
        K[1, 1] = intrinsics['K[1,1]']
        K[0, 2] = intrinsics['K[0,2]']
        K[1, 2] = intrinsics['K[1,2]']
        return K

    @staticmethod
    def _read_raw_calib_file(folder):
        """Read raw calibration files from folder."""
        file = (os.path.join(folder, "intrinsic.txt"))
        intrinsics = pd.read_csv(file, delimiter=" ")
...
########################################################################################################################
    ...
    def _read_depth(self, depth_file):
        ...
        if self.depth_type in ['groundtruth']:
            return imread(depth_file, IMREAD_ANYCOLOR | IMREAD_ANYDEPTH) / 100.
        else:
