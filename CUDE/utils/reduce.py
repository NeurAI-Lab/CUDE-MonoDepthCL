...
from CUDE.utils.horovod import reduce_value
from CUDE.utils.logging import prepare_dataset_prefix
...
def compute_cl_metrics(all_metrics):
    # Stack the metrics into n_tasks_completed x n_total_tasks x n_depth_metrics
    all_stacked_metrics = {}
    for key in all_metrics.keys():
        all_stacked_metrics[key] = np.stack(all_metrics[key])
    cl_metrics = {}
    # Loop over all keys (eg. gt, gt_pp) etc
    for metric_type in all_stacked_metrics.keys():
        for i in range(all_stacked_metrics[metric_type].shape[-1]): # Loop over the depth metrics (abs_rel,...)
            metrics = all_stacked_metrics[metric_type][:,:,i] # matrix of size n_tasks_completed x n_total_tasks

            # Metrics from paper Díaz-Rodríguez, Natalia, et al. "Don't forget, there is more than forgetting:
            # new metrics for Continual Learning." arXiv preprint arXiv:1810.13166 (2018)
            all_avg = np.average(metrics[np.tril_indices_from(metrics, k=0)]) # average of lower triangle

            final_avg = np.average(metrics[-1, :]) # average of last row

            bwt_matrix = np.array([metrics[i, :] - np.diagonal(metrics)[i] for i in range(len(np.diagonal(metrics)))])
            bwt = np.average(bwt_matrix[np.tril_indices_from(bwt_matrix, k=-1)]) # average of lower triangle without diagonal

            fwt = np.average(metrics[np.triu_indices_from(metrics, k=1)]) # average of upper triangle without diagonal

            diagonal_avg = np.average(np.diagonal(metrics)) # average of diagonal to measure plasticity

            if 'all_avg-' + metric_type not in cl_metrics.keys():
                cl_metrics['all_avg-' + metric_type] = []
            if 'final_avg-' + metric_type not in cl_metrics.keys():
                cl_metrics['final_avg-' + metric_type] = []
            if 'bwt-' + metric_type not in cl_metrics.keys():
                cl_metrics['bwt-' + metric_type] = []
            if 'fwt-' + metric_type not in cl_metrics.keys():
                cl_metrics['fwt-' + metric_type] = []
            if 'diagonal_avg-' + metric_type not in cl_metrics.keys():
                cl_metrics['diagonal_avg-' + metric_type] = []

            cl_metrics['all_avg-' + metric_type].append(all_avg)
            cl_metrics['final_avg-' + metric_type].append(final_avg)
            cl_metrics['bwt-' + metric_type].append(bwt)
            cl_metrics['fwt-' + metric_type].append(fwt)
            cl_metrics['diagonal_avg-' + metric_type].append(diagonal_avg)
...
def create_dict(metrics_data, metrics_keys, metrics_modes,
    ...
    for n, metrics in enumerate(metrics_data):
        if metrics: # If there are calculated metrics
            ...
            if len(name_split) > 1:
                prefix = f'{name_split[0]}_{prefix}'
            # For all keys
...
def create_cl_dict(metrics_data, metrics_keys):
    """
    Creates a dictionary from collated metrics
