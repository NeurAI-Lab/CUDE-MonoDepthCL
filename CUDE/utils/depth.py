...
from CUDE.utils.image import load_image, gradient_x, gradient_y, flip_lr, interpolate_image
from CUDE.utils.types import is_seq, is_tensor
...
def compute_depth_metrics(config, gt, pred, use_gt_scale=True, task_idx=0):
    """
    ...
    """
    crop = config.crop[task_idx] == 'garg'
    ...
    for pred_i, gt_i in zip(pred, gt):
        ...
        valid = (gt_i > config.min_depth[task_idx]) & (gt_i < config.max_depth[task_idx])
        ...
        pred_i = pred_i.clamp(config.min_depth[task_idx], config.max_depth[task_idx])
