...
from CUDE.utils.image import write_image
from CUDE.utils.depth import write_depth, inv2depth, viz_inv_depth
    ...
    if save.depth.rgb or save.depth.viz or save.depth.npz or save.depth.png:
        ...
        for i in range(length):
            ...
            if save.depth.rgb:
                ...
                write_image('{}/{}_rgb.png'.format(save_path, filename[i].replace("{}/", "")), rgb_i)
            # Save inverse depth visualizations
            if save.depth.viz:
                viz_i = viz_inv_depth(pred_inv_depth[i]) * 255
                write_image('{}/{}_viz.png'.format(save_path, filename[i].replace("{}/", "")), viz_i)
